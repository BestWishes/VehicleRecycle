...
<html>
	<head>
		...
		<script type="text/javascript">
			$(function() {
				$('#dg').datagrid({    
				    url:'${pageContext.request.contextPath}/storage/getstorages',  
				    method:'get',
				    idField:'id',
				    pagination:true,
				    cache:false,
				    ...
				    toolbar:[{
				    	  text: '添加', iconCls: 'icon-add', handler: function () {	
								$('#outdlstorage').dialog({
									href:'${pageContext.request.contextPath}/storage/add',
									method:'get',
									title:'新增仓库信息:',
									cache:false,
									width:380,    
									...
									buttons:[{
										...
										handler:function(){
											$('#storageform').form('submit',{
												url:'${pageContext.request.contextPath}/storage/save',
												method:'post',
												...
												success: function(data){
														$.messager.confirm('新增成功', '是否继续添加？', function(sure){
															if(sure){
																$('#outdlstorage').dialog('refresh');
															}
															else{
																$('#outdlstorage').dialog('close');
									                        	$('#dg').datagrid('reload'); 
															}
														})
												}
				    ...
				    columns:[[    
				              ...
				              {field:'storageName',title:'仓库名',rowspan:2,width:80,sortable:true},    
				              {field:'storageCapacity',title:'仓库容量',rowspan:2,width:80,sortable:true},    
				              {field:'storageAddress',title:'仓库地址',rowspan:2,width:80,sortable:true},    
		...
		function dgedit(rowid){
				$('#outdlstorage').dialog({
					href:'${pageContext.request.contextPath}/storage/edit/'+rowid,
					method:'get',
					title:'修改仓库信息:',
					cache:false,
					width:380,    
					...
					onLoad:function(){     //弹窗加载完成后执行
						...
						$('#storageform').form('load','${pageContext.request.contextPath}/storage/getedit/'+rowid);
					}
				});
			}
			 ...
			 function dgdelete(rowid){  //删除操作  
		            ...
		            $.messager.confirm('确认','id='+rowid+'\r确认删除?',function(sure){  
		                if(sure){  
		                    ...
		                    $.ajax({  
		                    	cache:false,
		                        url:'${pageContext.request.contextPath}/storage/delete/'+rowid,  
		                        method:'post',
		                        success:function(){
		                        	$.messager.show({
		                        	title:'提示',
