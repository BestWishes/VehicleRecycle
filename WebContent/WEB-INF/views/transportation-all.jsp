...
<html>
	<head>
		...
		<script type="text/javascript">
			$(function() {
				$('#dg').datagrid({    
				    url:'${pageContext.request.contextPath}/transportation/gettransportations',  
				    method:'get',
				    idField:'id',
				    pagination:true,
				    fitColumns:true,
				    ...
				    toolbar:[{
				    	text: '添加', iconCls: 'icon-add', handler: function () {	
							$('#outdl').dialog({
								href:'${pageContext.request.contextPath}/transportation/add',
								method:'get',
								title:'新增运输方式:',
								cache:false,
								width:400,    
								...
								buttons:[{
									...
									iconCls:'icon-save',
									handler:function(){
										$('#transportationform').form('submit',{
											url:'${pageContext.request.contextPath}/transportation/save',
											method:'post',
				    ...
				    columns:[[    
				              ...
				              {field:'name',title:'运输方式',rowspan:2,width:80,sortable:true},    
		...
		function dgedit(rowid){
				$('#outdl').dialog({
					href:'${pageContext.request.contextPath}/transportation/edit/'+rowid,
					method:'get',
					title:'修改运输方式:',
					cache:false,
					width:380,    
					...
					onLoad:function(){     //弹窗加载完成后执行
						$('#transportationform').form('load','${pageContext.request.contextPath}/transportation/getedit/'+rowid);
					}
				});
			}
			 ...
			 function dgdelete(rowid){  //删除操作  
		            ...
		            $.messager.confirm('确认','id='+rowid+'\r确认删除?',function(sure){  
		                if(sure){  
		                    ...
		                    $.ajax({  
		                    	cache:false,
		                        url:'${pageContext.request.contextPath}/transportation/delete/'+rowid,  
		                        method:'post',
		                        success:function(){
		                        	$.messager.show({
		                        	title:'提示',
	...
	<body>
		<div id="tdiv">
			...
			<div id="mm" style="width:120px"> 
				...
				<div data-options="name:'name',iconCls:'icon-ok'">按运输方式查询</div> 
			</div>
		</div>
		<div>
			<table id="dg"></table>  	
